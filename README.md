...
- `GET /documents` lists documents and supports optional `category` filtering.
- `GET /documents/{document_id}` returns one document or `404`.
- `POST /documents` creates a document and returns `201`.
- `PUT /documents/{document_id}` updates only submitted fields or returns `404`.
- `DELETE /documents/{document_id}` deletes one document or returns `404`.

## Swagger test checklist

- Create a document with a valid category.
- Confirm `id` and `created_at` are generated by the server.
- List all documents with `GET /documents`.
- Filter documents with `GET /documents?category=coding-convention`.
